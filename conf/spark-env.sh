...
# - this spark-env.sh
# - workers
# - log4j2.properties / metrics.properties / spark-defaults.conf
SPARK_CONF_DIR=${SPARK_CONF_DIR:-"${_this_conf_dir}"}
export SPARK_CONF_DIR

# Save logs (master/worker/history) under the project.
SPARK_LOG_DIR=${SPARK_LOG_DIR:-"${_project_root}/logs"}
export SPARK_LOG_DIR

# Keep pid files and worker state under the project (avoids /tmp surprises).
SPARK_PID_DIR=${SPARK_PID_DIR:-"${_project_root}/run"}
SPARK_WORKER_DIR=${SPARK_WORKER_DIR:-"${_project_root}/work"}
export SPARK_PID_DIR
export SPARK_WORKER_DIR

# Local disk for shuffle/RDD spill. Override if you have fast local NVMe.
SPARK_LOCAL_DIRS=${SPARK_LOCAL_DIRS:-"${_project_root}/tmp/spark-local"}
export SPARK_LOCAL_DIRS

# Standalone defaults (safe for single-node dev). Override as needed.
SPARK_MASTER_HOST=${SPARK_MASTER_HOST:-"localhost"}
SPARK_MASTER_PORT=${SPARK_MASTER_PORT:-17077}
...
SPARK_WORKER_PORT=${SPARK_WORKER_PORT:-17078}
